...
%DIF DEL paper_bioRxiv_v1.tex   Sat Jun  3 16:03:13 2023
%DIF ADD paper.tex              Sun Jun  4 20:16:51 2023
...
To anticipate and mitigate this evolution, the scientific community has launched efforts to assess the risk of new viral variants~\citep{degrace2022defining} and create therapeutics that target constrained regions of the virus where resistance is less likely to evolve~\DIFdelbegin %DIFDELCMD < \citep{moghadasi2022,iketani2022multiple,hiscox2021shutting}%%%
...
For spike, tractable lab assays have identified key functional and antigenic mutations~\citep{harvey2021sars,weisblum2020escape}, and enabled deep mutational scanning measurements of how most mutations affect receptor binding, cellular infection, and antibody recognition~\DIFdelbegin %DIFDELCMD < \citep{starr2020deep,dadonaite2022pseudovirus,greaney2021complete,cao2022imprinted}%%%
...
We also compute expected counts separately for each SARS-CoV-2 clade to account for shifts in mutation spectrum~\DIFdelbegin %DIFDELCMD < \citep{bloom2022evolution,ruis2022mutational}%%%
...
The expected counts per mutation \DIFdelbegin \DIFdel{(summed across all viral clades) }\DIFdelend vary with mutation type, ranging from $\sim$\DIFdelbegin \DIFdel{500 }\DIFdelend \DIFaddbegin \DIFadd{565 }\DIFaddend for \texttt{C$\rightarrow$T} to only $\sim$\DIFdelbegin \DIFdel{8 }\DIFdelend \DIFaddbegin \DIFadd{9 }\DIFaddend for \texttt{T$\rightarrow$G} mutations (Figure~\ref{fig:expected_vs_actual}B).
This variation is because the SARS-CoV-2 mutation spectrum is highly biased towards specific mutation types~\DIFdelbegin %DIFDELCMD < \citep{bloom2022evolution,ruis2022mutational,de2021mutation,neher2022contributions}%%%
\DIFdel{.
}\DIFdelend \DIFaddbegin \citep{bloom2023evolution,ruis2022mutational,de2021mutation,neher2022contributions}\DIFadd{.
However, because there are so many SARS-CoV-2 sequences we are able to estimate the rate of even the rarest mutation types with high accuracy~}\cite{bloom2023evolution}\DIFadd{.
For instance, there are $\sim 1.9\times 10^4$ observed occurrences of }\texttt{\DIFadd{T$\rightarrow$G}} \DIFadd{mutations across all $\sim$2,100 four-fold degenerate sites with a parental identity of }\texttt{\DIFadd{T}}\DIFadd{, which is enough to estimate the }\texttt{\DIFadd{T$\rightarrow$G}} \DIFadd{mutation rate (and therefore the expected counts of each mutation) with high accuracy.
}\DIFaddend 
...
But \DIFdelbegin \DIFdel{since }\DIFdelend the actual sampling intensity is $\sim$0.1\DIFdelbegin \DIFdel{--1\%the number of }\DIFdelend \DIFaddbegin \DIFadd{\%, since there are $\sim$7-million publicly available SARS-CoV-2 sequences and the total number of human infections is now probably roughly on par with the total global population of $\sim$8-billion people.
At this sampling intensity, the number of }\DIFaddend times a mutation is observed reflects more subtle reductions in transmission efficiency.
...
\subsection*{Mutation-effect estimates are robust to subsampling\DIFdelbegin \DIFdel{natural sequences}\DIFdelend , with some evidence of epistasis in spike}
...
\DIFaddbeginFL \DIFaddFL{Different subset size are also the reason why the regression line in (A) deviates from the identity $x=y$.
}\DIFaddendFL {\bf (C)} Correlations between clade or geography subsets become higher with an increasingly large threshold for minimum expected counts.
...
\DIFaddbegin \DIFadd{In particular, the interactive version of Figure~\ref{fig:corr}A shows that mutations that are more beneficial in Omicron BA.5 than Delta are often antibody-escape mutations (eg, K444N or G446S in spike~}\citep{greaney2022antibody}\DIFadd{)---a result that makes sense, since newer variants like Omicron BA.5 are evolving under increased immune selection compared to earlier variants like Delta that circulated in a more immunologically naive population.
}\DIFaddend 
...
\DIFaddbegin \DIFadd{An important question is whether the mutation fitness estimates are affected by noise from limited statistical sampling of mutations or whether sequencing errors and bioinformatic artefacts distort the estimates.
To assess if this is the case, we repeated the entire fitness estimation using an even larger pre-built UShER mutation-annotated tree of all $\sim$14-million SARS-CoV-2 sequences in GISAID~}\cite{shu2017gisaid} \DIFadd{as of March-29-2023.
...
Lastly, positions known to be under strong constraint }{\it \DIFadd{a priori}} \DIFadd{(e.g.~start codons, the ribosomal slippage site) typically have no or only few mutations, suggesting that sequencing errors in consensus sequences are rare.
}

\DIFaddend \subsection*{Structural and non-structural proteins are under strong purifying selection, but most accessory proteins are not}
...
\DIFaddbeginFL \DIFaddFL{The apparent constraint on synonymous mutation in ORF9b is probably because this gene is encoded in an overlapping reading frame with N~}\cite{jungreis2021conflicting}\DIFaddFL{.
}\DIFaddendFL See \url{https://jbloomlab.github.io/SARS2-mut-fitness/effects_histogram.html} and \url{https://jbloomlab.github.io/SARS2-mut-fitness/effects_dist.html} for plots that allow adjustment of the expected-count cutoff and other interactive options (such as separate histograms for each gene).
\DIFaddbeginFL \DIFaddFL{See Figure~\ref{fig:effects_dist_position_order} for a version of panel B with genes ordered by genomic position rather than constraint on nonsynonymous mutations.
}\DIFaddendFL \label{fig:dist}
}
\end{figure*}
...
\DIFaddbegin \DIFadd{The handful of synonymous mutations with highly deleterious effects are in either regions of known non-coding constraint (e.g., the ORF1ab ribosomal slippage site~}\cite{bhatt2021structural}\DIFadd{) or two regions in the center of E and the end of M (Figure~\ref{fig:synonymous}).
}\DIFaddend 

To investigate differences \DIFaddbegin \DIFadd{in functional constraint }\DIFaddend among viral proteins, we computed the distributions of \DIFaddbegin \DIFadd{mutation }\DIFaddend effects separately for each gene (Figure~\ref{fig:dist}B \DIFaddbegin \DIFadd{and \ref{fig:effects_dist_position_order}}\DIFaddend ).
...
The nonstructural and structural proteins are essential, and these proteins show strong selection against stop codons and clear although variable purifying selection against amino-acid mutations (Figure~\ref{fig:dist}B \DIFaddbegin \DIFadd{and \ref{fig:effects_dist_position_order}}\DIFaddend ; e.g., nsp13 is under stronger protein-level constraint than nsp1).

However, most accessory proteins are under little constraint (Figure~\ref{fig:dist}B \DIFaddbegin \DIFadd{and \ref{fig:effects_dist_position_order}}\DIFaddend ).
...
\DIFaddbegin \DIFadd{Indeed, the loss of accessory proteins such as ORF8 appears to occur with some regularity during the early evolution of non-human viruses in humans~}\citep{rochman2022molecular}\DIFadd{.
}\DIFaddend The only accessory protein under strong purifying selection against stop codons is ORF3a (Figure~\ref{fig:dist}B), for which stop codons in the first ~240 residues are clearly deleterious (Figure~\ref{fig:ORF3a}).
...
\caption{
Correlation of mutation-effect estimates with experimental deep mutational scanning measurements for {\bf (A)} the full spike~\DIFdelbeginFL %DIFDELCMD < \citep{dadonaite2022pseudovirus} %%%
\DIFdelendFL \DIFaddbeginFL \citep{dadonaite2023pseudovirus} \DIFaddendFL or its RBD~\citep{starr2022deep}, and {\bf (B)} Mpro~\DIFdelbeginFL %DIFDELCMD < \citep{flynn2022,iketani2022functional}%%%
\DIFdelendFL \DIFaddbeginFL \citep{flynn2023,iketani2022functional}\DIFaddendFL .
...
See \url{https://jbloomlab.github.io/SARS2-mut-fitness/dms_S_corr.html} and \url{https://jbloomlab.github.io/SARS2-mut-fitness/dms_nsp5_corr.html} for plots that \DIFdelbeginFL \DIFdelFL{enable subsetting on just mutations shared across all datasets }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{also show the Mpro dataset from~}\citep{flynn2022} \DIFaddendFL and \DIFdelbeginFL \DIFdelFL{other }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{have various }\DIFaddendFL interactive options\DIFdelbeginFL \DIFdelFL{such as mousing over points to see mutation identities}\DIFdelendFL .
The \DIFdelbeginFL \DIFdelFL{experiments in }%DIFDELCMD < \cite{starr2022deep, flynn2022} %%%
\DIFdelFL{measure multiple phenotypes and these }\DIFdelendFL plots \DIFaddbeginFL \DIFaddFL{in this figure }\DIFaddendFL show the \DIFdelbeginFL \DIFdelFL{effect }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{average }\DIFaddendFL of \DIFdelbeginFL \DIFdelFL{each mutation averaged across these }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{the multiple }\DIFaddendFL phenotypes \DIFaddbeginFL \DIFaddFL{measured in the deep mutational scanning of }\cite{starr2022deep}\DIFaddendFL ; see \url{https://jbloomlab.github.io/SARS2-mut-fitness/dms_S_all_corr.html} \DIFdelbeginFL \DIFdelFL{and }%DIFDELCMD < \url{https://jbloomlab.github.io/SARS2-mut-fitness/dms_nsp5_all_corr.html} %%%
\DIFdelendFL for \DIFdelbeginFL \DIFdelFL{plots that show }\DIFdelendFL each phenotype separately.
\DIFaddbeginFL \DIFaddFL{This figure only shows mutations with at least 20 expected counts, which is higher than the threshold of 10 used in most of the rest of this paper (this threshold can be adjusted in the interactive plots).
}\DIFaddendFL \label{fig:dms}
}
\end{figure*}
...
For spike, two distinct experimental methodologies have been used to characterize large numbers of mutations: yeast display of the receptor-binding domain (RBD)~\citep{starr2020deep,starr2022deep} and spike pseudotyped lentiviruses~\DIFdelbegin %DIFDELCMD < \citep{dadonaite2022pseudovirus}%%%
\DIFdelend \DIFaddbegin \citep{dadonaite2023pseudovirus}\DIFaddend .
For Mpro (also known as nsp5 or 3CLpro), two different labs have performed deep mutational scanning using the same basic methodology of assaying protease cleavage in yeast~\DIFdelbegin %DIFDELCMD < \citep{flynn2022,iketani2022functional}%%%
\DIFdelend \DIFaddbegin \citep{flynn2022,flynn2023,iketani2022functional}\DIFaddend .

For spike, our estimates from natural sequences correlate with \DIFdelbegin \DIFdel{each set of }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend experiments almost as well as the two experimental methodologies correlate with each other (Figure~\ref{fig:dms}A)\DIFdelbegin \DIFdel{.
If we increase the minimum expected counts from 10 to 20 and subset only on mutations shared among all three data sets, then the correlations between the }\DIFdelend \DIFaddbegin \DIFadd{, with Pearson correlations of 0.66 between the estimates and experiments versus 0.72 between the two experiments.
...
Therefore, it is unclear how much the differences between the mutation-effect }\DIFaddend estimates and experiments \DIFdelbegin \DIFdel{($r = 0.66$) become even closer to the cross-experiment correlations ($r = 0.72$; see interactive version of }\DIFdelend \DIFaddbegin \DIFadd{are due to noise in the estimates versus limitations of the experiments.
However, the fact that the estimates correlate with the experiments almost as well as the experiments correlate with each (}\DIFaddend Figure~\ref{fig:dms}A\DIFdelbegin \DIFdel{linked in legend) .
Some }\DIFdelend \DIFaddbegin \DIFadd{) suggests the estimates are of comparable quality to experimental measurements.
At least some }\DIFaddend of the mutations with the greatest divergence between our \DIFdelbegin \DIFdel{sequence-based }\DIFdelend estimates and the deep mutational scanning likely represent experimental artifacts.
...
The sequence-based estimates for Mpro also correlate with the deep mutational scans for that protein, although in this case the \DIFdelbegin \DIFdel{two }\DIFdelend experiments correlate substantially better with each other than with our estimates (Figure~\ref{fig:dms}B).
\DIFdelbegin \DIFdel{Because both Mpro experiments }\DIFdelend \DIFaddbegin \DIFadd{However, the Mpro experiments all }\DIFaddend use a similar yeast-based methodology~\DIFdelbegin %DIFDELCMD < \citep{flynn2022,iketani2022functional} %%%
\DIFdel{it is possible that the higher correlation of the experiments with each other than the sequence-based estimates reflects shared artifacts of the yeast experiments.
...
This difference could arise if the yeast experiments only capture some of the constraints on Mproin the context of actual virus}\DIFdelend \DIFaddbegin \citep{flynn2022,flynn2023,iketani2022functional} \DIFadd{that fails to capture significant aspects of Mpro's function during human infections}\DIFaddend .
For instance, a stop codon at Q306 is well tolerated in \DIFdelbegin \DIFdel{both }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend deep mutational scans but extremely disfavorable in our sequence-based estimates, and \DIFdelbegin \DIFdel{would }\DIFdelend \DIFaddbegin \DIFadd{such a mutation would clearly }\DIFaddend be highly deleterious to actual virus as it would truncate the polyprotein.
Similarly, K61N is well tolerated in the deep mutational scans but extremely disfavorable in our estimates, \DIFdelbegin \DIFdel{possibly }\DIFdelend \DIFaddbegin \DIFadd{probably }\DIFaddend because in the full viral polyprotein this residue mediates important interactions between Mpro and nsp7-10~\citep{yadav2022biochemical}

\DIFaddbegin \subsection*{\DIFadd{Mutation-effect estimates better capture functional constraint than dN/dS ratios or predictions from other methods}}
\DIFadd{A longstanding approach for analyzing protein constraint is to compare rates of nonsynonymous (dN) and synonymous (dS) substitutions at each site~}\citep{nielsen1998likelihood,kosakovsky2005not}\DIFadd{.
These dN/dS ratios can be calculated by counting mutations or using phylogenetic substitution models~}\citep{kosakovsky2005not,yang2000estimating}\DIFadd{.
A limitation of dN/dS ratios is they cannot be interpreted in terms of the fitness effects, since they simply represent the relative rate of amino-acid substitution rather than the effects of specific mutations~}\citep{spielman2015relationship,kryazhimskiy2008population}\DIFadd{.
...
For instance, the ACE2-affinity enhancing spike mutation N501Y arose in several SARS-CoV-2 variants early in the pandemic, and has since remained fixed due its importance for receptor binding~}\citep{starr2022deep}\DIFadd{.
...
\DIFadd{Our mutation-effect estimates also correlate better with deep mutational scanning experiments than predictions from two algorithms trained to learn epistatic models of mutation effects from phylogenetically broader but more sparsely sampled sequence data~}\cite{thadani2022learning,rodriguez2022epistatic} \DIFadd{(Figure~\ref{fig:comparator}).
Our estimates also correlate better with experiments than predictions by a machine-learning algorithm that integrates sequence and epidemiological data~}\cite{maher2022predicting} \DIFadd{(Figure~\ref{fig:comparator}).
...
First, because the number of observations of any given mutation is small compared to the millions of SARS-CoV-2 sequences being analyzed, our approach requires careful quality control \DIFdelbegin \DIFdel{of publicly available sequences to remove those affected by }\DIFdelend \DIFaddbegin \DIFadd{to remove }\DIFaddend sequencing errors.
...
Likewise, we neglect constraint on nucleotide identity beyond the encoded protein sequence~\citep{huston2021comprehensive,kuo2013functional}\DIFaddbegin \DIFadd{---although this probably has only a minor effect, since our analyses show just a handful of synonymous sites are under strong selection (Figure~\ref{fig:synonymous})}\DIFaddend .
...
The specific version of the repository used for this paper is tagged as ``\DIFdelbegin \DIFdel{bioRxiv-v1}\DIFdelend \DIFaddbegin \DIFadd{bioRxiv-v2}\DIFaddend '' on GitHub (\DIFdelbegin %DIFDELCMD < \url{https://github.com/jbloomlab/SARS2-mut-fitness/tree/bioRxiv-v1}%%%
...
\DIFaddbegin \subsection{\DIFadd{Versioning of analyses of different sequence sets}}
\DIFadd{The figures in this manuscript show analyses of the set of all publicly available sequences as of May-11-2023.
However, the pipeline can be run on different sequence sets.
The sequence sets on which the analysis is currently run include all sets listed under the ``mat\_trees'' key in }\url{https://github.com/jbloomlab/SARS2-mut-fitness/blob/main/config.yaml}\DIFadd{; these include the sets of all public sequences from several earlier dates (such as those available for the first version of this analysis), as well as the set of all sequences in GISAID as of March-29-2023.
A version of the results for each sequence set is provided in the GitHub repository (}\url{https://github.com/jbloomlab/SARS2-mut-fitness} \DIFadd{) in subdirectories with names like ``results\_public\_2023-05-11'', and the index page for the interactive plots (}\url{https://jbloomlab.github.io/SARS2-mut-fitness}\DIFadd{) links at the bottom to plots for each sequence set.
The ``current\_mat'' key in }\url{https://github.com/jbloomlab/SARS2-mut-fitness/blob/main/config.yaml} \DIFadd{specifies which sequence set is used to generate the main results that in the ``results'' subdirectory in the GitHub repository and are shown by default in the interactive plats; we anticipate periodically updating this to newer sequence sets as more sequences become available.
See }\url{https://jbloomlab.github.io/SARS2-mut-fitness/mat_aa_fitness_correlations.html} \DIFadd{for the correlations among mutation effects estimated from the different sequence sets.
}

\DIFadd{For the GISAID sequence set, we acknowledge the submitters of the sequences listed at the following URLs: }\url{https://doi.org/10.55876/gis8.230403ab}\DIFadd{, }\url{https://doi.org/10.55876/gis8.230403hg}\DIFadd{, }\url{https://doi.org/10.55876/gis8.230403ht}\DIFadd{, and }\url{https://doi.org/10.55876/gis8.230403tg}\DIFadd{.
}

\DIFaddend \subsection{Counting mutations along the phylogenetic tree}
We counted occurrences of each mutation in each viral clade using the UShER pre-built mutation-annotated tree~\citep{mcbroome2021daily,turakhia2021ultrafast,lanfear2020} from \DIFdelbegin \DIFdel{Dec-18-2022 (}%DIFDELCMD < \url{http://hgdownload.soe.ucsc.edu/goldenPath/wuhCor1/UShER_SARS-CoV-2/2022/12/18/public-2022-12-18.all.masked.nextclade.pangolin.pb.gz}%%%
...
\end{itemize}
The rationale for the first exclusion is that highly mutated branches are often indicative of sequencing errors \DIFdelbegin \DIFdel{, and the }\DIFdelend \DIFaddbegin \DIFadd{of viral evolution in chronically infected humans, neither of which correspond to the pattern of typical SARS-CoV-2 transmission in acute infections.
...
The }\DIFaddend rationale for the second and third exclusions is that excess reversions can arise from base-calling pipelines that erroneously call low-coverage sites as reference.
...
To calculate the expected counts for each nucleotide mutation, we analyzed just the four-fold degenerate sites in each clade in an approach paralleling that of \DIFdelbegin %DIFDELCMD < \cite{bloom2022evolution}%%%
...
Deep mutational scanning data were taken from published studies~\DIFdelbegin %DIFDELCMD < \citep{dadonaite2022pseudovirus,starr2022deep,flynn2022,iketani2022functional}%%%
\DIFdelend \DIFaddbegin \citep{dadonaite2023pseudovirus,starr2022deep,flynn2022,flynn2023,iketani2022functional}\DIFaddend , using the data at the links specified under the ``dms\_datasets'' key in \url{https://github.com/jbloomlab/SARS2-mut-fitness/blob/main/config.yaml}.
For the spike deep mutational scanning~\DIFdelbegin %DIFDELCMD < \citep{dadonaite2022pseudovirus} %%%
\DIFdelend \DIFaddbegin \citep{dadonaite2023pseudovirus} \DIFaddend we only included mutations with ``times seen'' values of at least three in the deep mutational scanning.
...
\DIFdelbegin \DIFdel{In both cases, Figure~\ref{fig:dms}}\DIFdelend \DIFaddbegin \DIFadd{Figures~\ref{fig:dms}, \ref{fig:dnds}, and \ref{fig:comparator} }\DIFaddend shows the effect averaged across all phenotypes measured by \DIFdelbegin \DIFdel{the study}\DIFdelend \DIFaddbegin \DIFadd{each of these studies}\DIFaddend .
...
\DIFaddbegin \subsection{\DIFadd{Comparison to dN/dS and other mutation-effect prediction algorithms}}
\DIFadd{For the comparison to the dN/dS approaches shown in Figure~\ref{fig:dnds}, we used the dN/dS values available at }\url{https://github.com/spond/SARS-CoV-2-variation}\DIFadd{~}\cite{martin2021emergence} \DIFadd{for all SARS-CoV-2 sequences, which were calculated using the FEL approach~}\cite{kosakovsky2005not}\DIFadd{.
...
\DIFadd{We also compared both our mutation-effect estimates and the spike deep mutational scanning measurements~}\citep{dadonaite2023pseudovirus} \DIFadd{to predictions from three other algorithms:
}\begin{itemize}
\item \DIFadd{the EpiScores reported by Maher et al (2022)~}\cite{maher2022predicting}\DIFadd{,
}\item \DIFadd{the DCA mutability scores reported by Rodriguez-Rivas et al (2022)~}\cite{rodriguez2022epistatic}\DIFadd{, and
}\item \DIFadd{the EVE scores reported by Thadani et al (2023)~}\cite{thadani2022learning}\DIFadd{.
}\end{itemize}
\DIFadd{These comparisons are shown in Figure~\ref{fig:comparator}.
...
\DIFaddend \subsection{Derivation of relationship between actual to expected count ratio and viral fitness}
...
We thank \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{Angie Hinrichs for providing the pre-built mutation-annotated trees and thank the }\DIFaddend UShER team for promptly answering and addressing GitHub issues related to use of this package and its pre-built \DIFdelbegin \DIFdel{mutation-annotated tree.
}\DIFdelend \DIFaddbegin \DIFadd{trees.
We thank the sequence submitters to GISAID, who are listed in the tables cited in the Methods section.
}\DIFaddend The work of JDB was supported in part by the NIH/NIAID under grants U19AI171399 and R01AI141707, and contract 75N93021C00015.
JDB is an Investigator of the Howard Hughes Medical Institute.

\section{Competing interests}
JDB \DIFdelbegin \DIFdel{is on the scientific advisory boards of }\DIFdelend \DIFaddbegin \DIFadd{consults }\DIFaddend Apriori Bio, Aerium Therapeutics, Invivyd, the Vaccine Company, \DIFdelbegin \DIFdel{and Oncorus}\DIFdelend \DIFaddbegin \DIFadd{GSK, and Pfizer on topics related to viral evolution}\DIFaddend .
...
\caption{
\DIFaddFL{Correlation between amino-acid fitness estimates from the $\sim$7-million sequence set of all publicly available sequences as of May-11-2023, or the set of all $\sim$14-million sequences in GISAID as of March-29-2023.
...
See }\url{https://jbloomlab.github.io/SARS2-mut-fitness/mat_aa_fitness_correlations.html} \DIFaddFL{for an interactive version of this plot that also shows the correlations for other sets of publicly available sequences.
...
\caption{
{\bf \DIFaddFL{Non-coding constraint is concentrated in a few isolated regions in SARS-CoV-2 genome.}}
{\bf \DIFaddFL{(A), (B)}} \DIFaddFL{The average fitness cost of synonymous mutations (do not change amino acids) in ORF1ab, S, E, M, N, and ORF3a, non-coding regions between ORFs, and all positions in ORF6, ORF7a, ORF7b, ORF8, ORF10 (as these ORFs tolerate stop codons, see Figure~\ref{fig:dist}B).
...
Regions with large fitness costs for non-coding mutations correspond to the ribosomal slippage site at the end of ORF1a, the transcription regulatory sites of S, ORF3a, E, M, and N, as well as two regions in the center of E and the end of M (see red bars in the top part of each panel) }\citep{markov2023evolution}\DIFaddFL{.
...
}{\bf \DIFaddFL{(C), (D)}} \DIFaddFL{The distribution of inferred fitness cost of four-fold degenerate sites at regions of known non-coding constraint within the protein coding genes (the ribosomal slippage and TRS sites) versus all other four-fold degenerate sites.
...
\caption{
\DIFaddFL{A version of Figure~\ref{fig:dist}B but with genes ordered by position in the genome rather than extent of constraint on nonsynonymous mutations.
See }\url{https://jbloomlab.github.io/SARS2-mut-fitness/effects_dist_position_order.html} \DIFaddFL{for an interactive version of this plot.
...
\begin{figure*}[h]
\DIFaddendFL \includegraphics[width=\linewidth]{figs/ORF3a.pdf}
\caption{
...
\DIFdelbeginFL \DIFdelFL{This plot shows only mutations with 20 expected counts.
}\DIFdelendFL See \url{https://jbloomlab.github.io/SARS2-mut-fitness/ORF3a.html} for an interactive version of this plot along with zoomable heatmap of the effects of specific amino-acid substitutions.
\label{fig:ORF3a}
}
\end{figure*}
...
\caption{
\DIFaddFL{The site-mean of the estimated fitness effects outperforms dN/dS ratios as an indicator of functional constraint as assessed by correlation with deep mutational scanning measurements.
}{\bf \DIFaddFL{(A)}}
\DIFaddFL{Pearson correlation of log dN/dS ratio at each site with the mean estimated fitness of all amino acid mutations at each site from the current study.
...
\DIFaddFL{Correlation of the log dN/dS ratio or the mean estimated fitness effect at each site with the mean impact of amino-acid mutations at that site as experimentally measured by deep mutational scanning.
...
The dN/dS ratios are taken from }\url{https://github.com/spond/SARS-CoV-2-variation}\DIFaddFL{.
See }\url{https://jbloomlab.github.io/SARS2-mut-fitness/dnds_corr.html} \DIFaddFL{for an interactive version of this plot.
}\label{fig:dnds}
}
\end{figure*}
...
\caption{
\DIFaddFL{The mutation effect estimates from the current study are substantially better correlated with deep mutational scanning measurements for spike than predicted mutation effects made using three other approaches.
...
See }\url{https://jbloomlab.github.io/SARS2-mut-fitness/comparator_corr.html} \DIFaddFL{for an interactive version of this plot.
...
\caption{
\DIFaddFL{Number of mutations per branch on all branches of the SARS-CoV-2 tree with at least one mutation.
