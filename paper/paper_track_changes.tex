...
%DIF ADD paper.tex              Sat Jun  3 06:46:45 2023
...
\subsection*{Mutation-effect estimates are robust to subsampling \DIFdelbegin \DIFdel{natural sequences}\DIFdelend \DIFaddbegin \DIFadd{and phylogeny}\DIFaddend , with some evidence of epistasis in spike}
...
%DIF > We also note that the fact that two trees (which have independently inferred topologies) give nearly identical fitness estimates indicates that uncertainty in tree topology has minimal effect.
%DIF >  The insensitivity of our fitness estimates to uncertainty in tree topology is expected: the main uncertainty in densely sampled viral phylogenies involves polytomies of closely related sequences, but our estimates only use the counts of mutations along tree branches, which are unaffected by uncertainty in the resolution of polytomies of closely related sequences.
}
...
To investigate differences \DIFaddbegin \DIFadd{in functional constraint }\DIFaddend among viral proteins, we computed the distributions of \DIFaddbegin \DIFadd{mutation }\DIFaddend effects separately for each gene (Figure~\ref{fig:dist}B).
...
These dN/dS ratios can be calculated by counting mutations or using phylogenetic substitution models~}\citep{kosakovsky2005not}\DIFadd{.
