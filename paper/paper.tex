...
To anticipate and mitigate this evolution, the scientific community has launched major efforts to assess the risk of new viral variants and create therapeutics that target constrained regions of the virus where resistance is less likely to evolve~\citep{?}.
...
For spike, tractable laboratory assays have identified key functional and antigenic mutations~\citep{?}, and enabled deep mutational scanning measurements of how most mutations affect receptor binding, cellular infection, and antibody recognition~\citep{?}.
These experimental data are valuable for assessing new spike variants and informing development of antibody therapeutics with greater resistance to escape~\citep{?}.
...
The amount of data available for such analyses has increased dramatically over the last few years with the full-genome sequencing of SARS-CoV-2 from millions of human infections~\citep{?}.
...
The basic idea of these prior analyses is that mutations that repeatedly appear near the base of clades that expand in relative frequency are likely beneficial to the virus.
...
Here we develop a new approach that uses natural sequences to estimate the effects of mutations to SARS-CoV-2.
Our basic insight is that there are now so many SARS-CoV-2 sequences that all non-deleterious single-nucleotide mutations are expected to independently occur tens to hundreds of times along the observed phylogeny.
We therefore first calculate the number of \emph{expected} occurrences of each mutation based on the neutral mutation spectrum of SARS-CoV-2 as estimated from four-fold degenerate synonymous sites~\citep{?}.
We then compare these expected occurrences to the actual occurrences in the SARS-CoV-2 phylogeny to estimate the effect of each mutation.
The resulting estimates are used to create maps of mutation-level functional constraint across each viral protein, which correlate well with existing deep mutational scanning data.
Most viral proteins have regions that are under strong selective constraints.
However the accessory proteins show only weak selection against amino-acid and even stop-codon mutations, with the main exception being strong selection against premature stop codons in most of ORF3a.
