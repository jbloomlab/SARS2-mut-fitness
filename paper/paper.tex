...
To assess if this is the case, we repeated the entire fitness estimation using an even larger pre-built UShER mutation-annotated tree of all $\sim$14-million SARS-CoV-2 sequences in GISAID as of March-29-2023.
...
This concordance indicates that the $\sim$7-million public sequence set is large enough that statistical sampling noise is low, and so throughout this paper we use that sequence set due to our preference for publicly available data.
We also note that the fact that two trees (which have independently inferred topologies) give nearly identical estimates indicates that uncertainty in tree topology has minimal effect on our fitness estimates.
...
\caption{
Correlation of mutation-effect estimates with experimental deep mutational scanning measurements for {\bf (A)} the full spike~\citep{dadonaite2023pseudovirus} or its RBD~\citep{starr2022deep}, and {\bf (B)} Mpro~\citep{flynn2022,iketani2022functional}.
...
\caption{
\jdbcomment{write}
\url{https://jbloomlab.github.io/SARS2-mut-fitness/dnds_corr.html}
\label{fig:dnds}
}
\end{figure*}
...
\caption{
\jdbcomment{write}
\url{https://jbloomlab.github.io/SARS2-mut-fitness/comparator_corr.html}
