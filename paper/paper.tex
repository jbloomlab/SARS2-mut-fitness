...
The statistical noise in the count ratios is greater for mutations with fewer expected counts: the figures in this paper show only mutations with $\ge10$ expected counts, with the legends linking to interactive plots that enable adjustment of this threshold.
...
It is not obvious why there should be real differences in mutation effects in geographic locations, but there is experimental evidence that epistasis has shifted effects of some spike mutations during viral evolution~\citep{starr2022shifting,moulana2022compensatory,mccallum2021n}.
...
However, for the rest of this paper we aggregate counts across clades, since for most purposes the benefits aggregating counts (which enables lower-noise estimates more mutations) outweighs the effect of modest epistatic shifts.

\subsection*{Structural and non-structural proteins are under strong purifying selection, but many accessory genes are not}
