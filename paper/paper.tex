...
However, experimentally determining effects of mutations is challenging: we lack tractable lab assays for many SARS-CoV-2 proteins, and comprehensive deep mutational scanning has been applied to only two SARS-CoV-2 proteins.
...
Other studies have analyzed broader alignments that include coronaviruses substantially diverged from SARS-CoV-2~\citep{rodriguez2022epistatic, thadani2022learning}, but the resulting estimates are inherently limited by the fact that mutational effects shift as proteins diverge~\citep{starr2022shifting, moulana2022compensatory, pollock2012amino, shah2015contingency, lee2018deep}.
% \jdbcomment{Richard, I could not quite figure out how to fit in \citep{strelkowa2021clonal}, let me know if you have ideas.}

We previously estimated fitness costs of mutations in the HIV-1 genomes from deep sequencing data of within host diversity, where the frequency of a deleterious mutation in the viral quasi-species is determined by a balance between repeated mutation and purifying selection \citep{zanini2017vivo}.
But in SARS-CoV-2, mutation frequency is a poor predictor of its fitness effect since moderately deleterious mutations can hitch-hike on emerging variants.
...
We therefore first calculate the number of expected observations of independent occurrences of each mutation based on the neutral mutation spectrum of SARS-CoV-2 estimated from four-fold degenerate synonymous sites~\citep{bloom2022evolution}.
...
A key result from this derivation is the dependence on sampling intensity: if all human SARS-CoV-2 were sequenced even very deleterious mutations would have a high chance of being sampled and we would need to study the subsequent spread of the mutations to assess their fitness.
But since the actual sampling intensity is $\sim$0.1--1\% the number of times mutation is observed sensitively probes more subtle effects that transmit with reduced efficiency.
...
We thank Angie Hinrichs for assistance with questions related to use of the UShER package and its pre-built mutation-annotated tree \jdbcomment{check with her to make sure acknowledging is OK}.
...
\jdbcomment{Jesse needs to confirm permission of Chodera and Hensley to cite U19 and CEIRR contract.}
