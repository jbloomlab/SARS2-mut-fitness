...
\subsection*{Mutation-effect estimates are robust to subsampling and phylogeny, with some evidence of epistasis in spike}
...
To investigate differences in functional constraint among viral proteins, we computed the distributions of mutation effects separately for each gene (Figure~\ref{fig:dist}B).
...
For spike, our estimates from natural sequences correlate with the experiments almost as well as the two experimental methodologies correlate with each other (Figure~\ref{fig:dms}A), with Pearson correlations of 0.66 between the fitness estimates and experiments versus 0.72 between the two types of experiments.
Neither set of experimental measurements fully captures how mutations affect viral fitness, since both RBD yeast display and lentiviral pseudotyping are imperfect proxies for spike function during actual human infections.
Therefore, it is unclear how much the differences between the fitness estimates and experiments are due to noise in the fitness estimates versus limitations of the experiments.
However, the fact that the fitness estimates correlate with the experiments almost as well as experimental methodologies correlate with each (Figure~\ref{fig:dms}A) suggests estimates are of comparable quality to experimental measurements.
At least some of the mutations with the greatest divergence between our fitness estimates and the deep mutational scanning likely represent experimental artifacts.
...
The sequence-based fitness estimates for Mpro also correlate with the deep mutational scans for that protein, although in this case the experiments correlate substantially better with each other than with our estimates (Figure~\ref{fig:dms}B).
However, the Mpro experiments all use a similar yeast-based methodology~\citep{flynn2022,flynn2023,iketani2022functional} that probably fails to capture significant aspects of Mpro's actual function during human infections.
For instance, a stop codon at Q306 is well tolerated in the yeast-based Mpro deep mutational scans but extremely disfavorable in our sequence-based estimates, and such a mutation would clearly be highly deleterious to actual virus as it would truncate the polyprotein.
...
These dN/dS ratios can be calculated by counting mutations or using phylogenetic substitution models~\citep{kosakovsky2005not}.
A known limitation of dN/dS ratios is they cannot be interpreted in terms of the fitness effects, since they simply represent the relative rate of amino-acid substitution rather than the effects of specific mutations~\citep{spielman2015relationship,kryazhimskiy2008population}.
...
Our mutation-effect estimates also correlate substantially better with deep mutational scanning experiments than predictions from two methods trained to learn mutation effects and epistatic interactions from phylogenetically broader but more sparsely sampled sequence data~\cite{thadani2022learning,rodriguez2022epistatic} (Figure~\ref{fig:comparator}).
Our estimates also correlate better with experiments than predictions of a machine-learning method that integrates a wide range of sequence and epidemiological data~\cite{maher2022predicting} (Figure~\ref{fig:comparator}).
These results suggest that our relatively straightforward approach of directly reading out the effects of mutations from their actual versus expected counts can outperform much more complex models when millions of sequences are available. 
...
Deep mutational scanning data were taken from published studies~\citep{dadonaite2023pseudovirus,starr2022deep,flynn2022,iketani2022functional}, using the data at the links specified under the ``dms\_datasets'' key in \url{https://github.com/jbloomlab/SARS2-mut-fitness/blob/main/config.yaml}.
