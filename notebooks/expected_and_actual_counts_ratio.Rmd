...
To decrease the amount of time it takes to run the contingency tests, add columns to the filtered comparisons data frame with the observed and expected values for clade 1 and clade 2.

```{r contingency_matrix}
# Add observed and expected values for each comparison to filtered comparisons data frame.
{filtered_comparisons$clade1_observed <- fisher_data[which(fisher_data$nt_mutation_clade %in% filtered_comparisons$clade1),"observed"]
filtered_comparisons$clade1_expected <- fisher_data[which(fisher_data$nt_mutation_clade %in% filtered_comparisons$clade1),"expected"]
filtered_comparisons$clade2_observed <- fisher_data[which(fisher_data$nt_mutation_clade %in% filtered_comparisons$clade2),"observed"]
filtered_comparisons$clade2_expected <- fisher_data[which(fisher_data$nt_mutation_clade %in% filtered_comparisons$clade2),"expected"]}

#
{filtered_comparisons$clade1_obs_exp <- fisher_data[which(fisher_data$nt_mutation_clade %in% filtered_comparisons$clade1),"observed_expected"]
filtered_comparisons$clade2_obs_exp <- fisher_data[which(fisher_data$nt_mutation_clade %in% filtered_comparisons$clade2),"observed_expected"]}
```
...
# NEED TO CORRECT FOR MORE THAN 2 CLADE COMPARISONS
comparison_p_values$p_values_corrected <- comparison_p_values$p_value/nrow(comparison_p_values)
```
