...
fisher_data <- data.frame(nt_mutation_clade=paste(clade_SARS_mutation$nt_mutation, clade_SARS_mutation$clade, sep="_"), observed=clade_SARS_mutation$actual_count, expected=clade_SARS_mutation$rounded_expected, observed_expected=clade_SARS_mutation$observed_expected)
```
...
rep_nt_mutation <- data.frame(nt_mutation=rep(unique(filtered_SARS_mutation$nt_mutation), each=nrow(clade_comparisons)))
...
rep_clades <- do.call("rbind", replicate(length(unique(filtered_SARS_mutation$nt_mutation)), clade_comparisons, simplify=FALSE))
...
comparison_p_values$fold_change <- log2(ifelse(comparison_p_values$clade1_obs_exp >= comparison_p_values$clade2_obs_exp, comparison_p_values$clade1_obs_exp/comparison_p_values$clade2_obs_exp, -comparison_p_values$clade2_obs_exp/comparison_p_values$clade1_obs_exp))
```
...
ggplot(comparison_p_values, aes(x=other_fold, y=-log10(p_values_corrected)))+
  geom_point()
```
