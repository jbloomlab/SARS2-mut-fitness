...
# Remove columns that were already used to filter the data or than contain redundant information.
filtered_clades$subset <- NULL
filtered_clades$exclude <- NULL
filtered_clades$nt_site <- NULL
```
...
colnames(clade1) <- paste("c1", colnames(clade1), sep="_")
colnames(clade2) <- paste("c2", colnames(clade2), sep="_")

# Modify nt_mutation, clade_founder_nt and clade to make merging easier and make the data frame more readable.
{colnames(clade1)[which(colnames(clade1)=="c1_nt_mutation")] <- "nt_mutation"
colnames(clade2)[which(colnames(clade2)=="c2_nt_mutation")] <- "nt_mutation"
colnames(clade1)[which(colnames(clade1)=="c1_clade_founder_nt")] <- "clade_founder_nt"
colnames(clade2)[which(colnames(clade2)=="c2_clade_founder_nt")] <- "clade_founder_nt"
colnames(clade1)[which(colnames(clade1)=="c1_clade")] <- "clade1"
colnames(clade2)[which(colnames(clade2)=="c2_clade")] <- "clade2"}

# Merge the two data frames by mutation so that each row represents one comparison.
comparisons <- merge(clade1, clade2, by=c("nt_mutation","clade_founder_nt"))
```
...
p_values <- data.frame(p_value=apply(comparisons, 1, FUN=function(c) {fisher.test(rbind(c(as.numeric(c["c1_actual_count"]),as.numeric(c["c1_rounded_expected"])),c(as.numeric(c["c2_actual_count"]),as.numeric(c["c2_rounded_expected"]))))$p.value}))
...
comparison_p_values$fold_change <- log2(comparison_p_values$c1_observed_expected/comparison_p_values$c2_observed_expected)
```
...
ggplot(volcano, aes(x=fold_change, y=-log10(p_value_corrected), label=nt_mutation, color=clade_founder_nt))+
  geom_point()+
...
verify_analyses$synonymous <- ifelse(verify_analyses$c1_synonymous=="TRUE" & verify_analyses$c2_synonymous=="TRUE", "TRUE", "FALSE")
