...
'p_values <- data.frame(p_value=apply(fisher_data, 1, FUN=function(c) {fisher.test(rbind(c(as.numeric(c[3]),as.numeric(c[4])),c(as.numeric(c[5]),as.numeric(c[6]))))$p.value}))'
...
comparison_p_values <- cbind(fisher_data, p_values)
```
...
comparison_p_values$fold_change <- log2(ifelse(comparison_p_values$clade1_observed_expected >= comparison_p_values$clade2_observed_expected, comparison_p_values$clade1_observed_expected/comparison_p_values$clade2_observed_expected, -comparison_p_values$clade2_observed_expected/comparison_p_values$clade1_observed_expected))
```
...
#plot_data <- comparison_p_values[which(comparison_p_values$clades=="20A.22A"),]
ggplot(comparison_p_values, aes(x=fold_change, y=-log10(p_values_corrected)))+
  geom_point()
```
...
plot_data <- comparison_p_values[which(comparison_p_values$clade1_four_fold_degenerate=="TRUE"),]
nb <- sqrt(nrow(plot_data))
bw <- (max(comparison_p_values$p_values_corrected)-min(comparison_p_values$p_values_corrected))/nb
ggplot(plot_data, aes(x=p_values_corrected))+
  geom_histogram(bins = nb, binwidth = bw)

ggplot(comparison_p_values[which(comparison_p_values$clade1_four_fold_degenerate=="TRUE"),], aes(x=p_values_corrected, y=p_values_corrected))+
  geom_point()
```

## Shorted Script That Can Handle 2 Clades
```{r two_clades}
...
clade1 <- clade_SARS_mutation[which(clade_SARS_mutation$clade==clades[1]),c("nt_mutation","clade","actual_count","expected_count","observed_expected")]
clade2 <- clade_SARS_mutation[which(clade_SARS_mutation$clade==clades[2]),c("nt_mutation","clade","actual_count","expected_count","observed_expected")]
m <- merge(clade1, clade2, by="nt_mutation")
filtered_comparisons <- data.frame(clade1=paste(m$nt_mutation, m$clade.x, sep="_"), clade2=paste(m$nt_mutation, m$clade.y, sep="_"), clade1_observed=m$actual_count.x, clade1_expected=m$expected_count.x, clade2_observed=m$actual_count.y, clade2_expected=m$expected_count.y, clade1_obs_exp=m$observed_expected.x, clade2_obs_exp=m$observed_expected.y)
end_two <- Sys.time()}
end_two - start_two
two <- filtered_comparisons'
```
