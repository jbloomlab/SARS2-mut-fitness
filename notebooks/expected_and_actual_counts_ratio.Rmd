...
```{r aa_mutation}
if(option=='aa'){
  aa_group <- filtered_SARS_mutation
  filtered_SARS_mutation <- as.data.frame(aa_group %>% group_by(clade, aa_mutation, clade_founder_aa, codon_site, mutant_aa, synonymous) %>% summarise(actual_count=sum(actual_count), expected_count=sum(expected_count)))
}else{
  nt_group <- filtered_SARS_mutation
  filtered_SARS_mutation <- nt_group[,c("clade","nt_mutation","clade_founder_nt","nt_site","synonymous","four_fold_degenerate","actual_count","expected_count")]
}
...
aa_type <- paste(filtered_SARS_mutation$clade_founder_aa, ".", filtered_SARS_mutation$mutant_aa, sep="")
...
stacked_col <- colorRampPalette(brewer.pal(8,"Set1"))(79)
...
ggplot(stacked_aa, aes(x=clade, y=actual_count/expected_count, fill=mutant_aa))+
  geom_bar(position="stack", stat="identity")+
  facet_wrap(~clade_founder_aa, ncol=7)+
...
color <- colorRampPalette(brewer.pal(8,"Set1"))(comparison_p_values[,paste("clade_founder", option, sep="_")] %>% unique() %>% length())

volcano <- comparison_p_values[which(comparison_p_values$p_value_corrected!=1),]
ggplot(volcano, aes(x=fold_change, y=-log10(p_value_corrected), label=get(merge_column), color=get(paste("clade_founder", option, sep="_"))))+
  geom_point()+
