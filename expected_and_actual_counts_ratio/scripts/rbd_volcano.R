...
merge_column <- "aa_mutation"

# Create filename for plotting mutation data from a two-clade comparison.
input_name <- paste("results/pvalues", snakemake@wildcards[["comparisons"]], "_aa.csv", sep="")

# Read in the data for plotting.
comparison_p_values <- read.csv(input_name, sep=",", header=TRUE)

# Filter amino acid mutations for RBD (331 to 530).
spike <- comparison_p_values[which(comparison_p_values$gene=="S" & comparison_p_values$codon_site>=331 & comparison_p_values$codon_site<=530),]
comparison_p_values <- spike
...
if(is.null(x)){
  ...
  p + annotation_custom(neg_clade,xmin=x_mod[1]*0.7,xmax=x_mod[1]*0.7,ymin=y[2]*0.95,ymax=y[2]*0.95) + annotation_custom(pos_clade,xmin=x_mod[2]*0.7,xmax=x_mod[2]*0.7,ymin=y[2]*0.95,ymax=y[2]*0.95)
}
