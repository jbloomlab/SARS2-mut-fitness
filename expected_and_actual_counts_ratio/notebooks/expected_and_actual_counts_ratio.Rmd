...
p <- ggplot(comparison_p_values, aes(x=fold_change, y=-log10(p_value)))+
...
ggplot(volcano, aes(x=fold_change, y=-log10(p_value_corrected), label=get(merge_column), color=gene))+
  geom_point()+
...
Plot the same volcano plot but color only the points where the mutation is in the spike protein for one or both clades.

```{r volcano_corrected_spike, echo=FALSE}
...
ggplot(volcano, aes(x=fold_change, y=-log10(p_value_corrected), label=get(merge_column), color=spike))+
  geom_point()+
  geom_text(color="black", size=2, nudge_y=0.2)+
  geom_hline(yintercept=-log10(0.05), col="red", linetype=2)+
  scale_color_manual(values=color, name="mutation")+
